...
$ cmake -B bolt-build -G Ninja llvm-project/llvm \
   ...
   -DLLVM_EXTERNAL_PROJECTS="bolttests-arm" \
   -DLLVM_EXTERNAL_BOLTTESTS_ARM_SOURCE_DIR=$(pwd)/large-bolt-tests
$ cmake --build bolt-build --target check-large-bolt-arm
```

When this repo is configured as an external project, it will add itself as an
extra target in LLVM named "check-large-bolt-arm". Just build that target to run
this testsuite.
